...
export default function ProfilePanel() {
  ...
  useEffect(() => {
    getUserProfile().then(setProfile).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <ScrollView style={s.scroll} contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
      <View style={s.profileRow}>
        ...
        <View style={s.profileInfo}>
          ...
          <Text style={s.joined}>항해 시작 {profile.joined}</Text>
        </View>
      ...
      <View style={s.statRow}>
        <View style={s.statCard}>
          <Text style={s.statValue}>{profile.totalVoyages}회</Text>
          <Text style={s.statLabel}>총 항해</Text>
        </View>
        <View style={s.statCard}>
          <Text style={s.statValue}>{profile.visitedCities}곳</Text>
      ...
      <Modal visible={donateOpen} transparent animationType="fade" onRequestClose={() => setDonateOpen(false)}>
        <Pressable style={s.donateOverlay} onPress={() => setDonateOpen(false)}>
          <Pressable style={s.donateCard} onPress={() => {}}>
            <View style={s.donateIcon}><Text style={s.donateIconText}>⛵</Text></View>
            ...
            <Pressable onPress={() => setDonateOpen(false)} style={{ marginTop: 18 }}>
          ...
          <View style={s.editInner}>
            ...
            <View style={s.editBody}>
              {(!isLocal || tab === "nickname") ? (
                ...
                <View style={{ gap: 10 }}>
                  {/* 비번 보기 토글 — secureTextEntry 자동완성 간섭 회피 */}
                  ...
                  {/* iOS 자동완성 디코이 — 첫 비번 필드 잠금을 이 더미가 흡수 */}
                  <TextInput style={{ height: 0, width: 0, opacity: 0 }} autoComplete="off" />
                  <TextInput value={pwCurrent} onChangeText={(t) => { console.log("[현재비번 입력]", t.length); setPwCurrent(t); }} secureTextEntry={!showPw} textContentType="oneTimeCode" autoComplete="off" autoCapitalize="none" autoCorrect={false} placeholder="현재 비밀번호" placeholderTextColor="#1a3a50" style={s.input} />
                  <TextInput value={pwNext} onChangeText={setPwNext} secureTextEntry={!showPw} textContentType="oneTimeCode" autoComplete="off" autoCapitalize="none" autoCorrect={false} placeholder="새 비밀번호 (8자 이상)" placeholderTextColor="#1a3a50" style={s.input} />
                  <TextInput value={pwConfirm} onChangeText={setPwConfirm} secureTextEntry={!showPw} textContentType="oneTimeCode" autoComplete="off" autoCapitalize="none" autoCorrect={false} placeholder="새 비밀번호 확인" placeholderTextColor="#1a3a50" style={s.input} />
                  <View style={s.rowBetween}>
...
const s = StyleSheet.create({
  ...
  statValue: { color: "#4a9abb", fontSize: 22, fontWeight: "500" },
  ...
  donateIconText: { fontSize: 18, color: "#7eb8d4" },
