...
const root = path.resolve(__dirname, '../src/components/r3f/daejeon')
const { buildExpoNeighborhood, buildExpoLandmark } = load(path.join(root, 'ExpoPlaza.ts'))
fs.mkdirSync(path.join(root, 'baked'), { recursive: true })
for (const portrait of [false, true]) {
  for (const [name, build] of [['plaza', buildExpoNeighborhood], ['tower', buildExpoLandmark]]) {
