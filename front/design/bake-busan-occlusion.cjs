...
const root = path.resolve(__dirname, '../src/components/r3f/busan')
const { buildBusanLandmark, buildBusanNeighborhood } = load(path.join(root, 'HarborNeighborhood.ts'))
fs.mkdirSync(path.join(root, 'baked'), { recursive: true })
for (const portrait of [false, true]) {
  for (const [name, build] of [['tower', buildBusanLandmark], ['town', buildBusanNeighborhood]]) {
