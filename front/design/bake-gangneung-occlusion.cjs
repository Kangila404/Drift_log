...
const root = path.resolve(__dirname, '../src/components/r3f/gangneung')
const { buildGangneungCoast, buildGangneungLandmark } = load(path.join(root, 'HeadlandTerrace.ts'))
const { buildGangneungQuarter } = load(path.join(root, 'CoastalQuarter.ts'))
fs.mkdirSync(path.join(root, 'baked'), { recursive: true })
for (const portrait of [false, true]) {
  for (const [name, build] of [['coast', buildGangneungCoast], ['hotel', buildGangneungLandmark], ['quarter', buildGangneungQuarter]]) {
