...
const root = path.resolve(__dirname, '../src/components/r3f/suwon')
const { buildSuwonLandmark, buildSuwonNeighborhood } = load(path.join(root, 'InlandNeighborhood.ts'))
fs.mkdirSync(path.join(root, 'baked'), { recursive: true })
for (const portrait of [false, true]) {
  for (const [name, build] of [['gate', buildSuwonLandmark], ['town', buildSuwonNeighborhood]]) {
