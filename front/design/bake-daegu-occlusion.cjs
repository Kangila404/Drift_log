...
const root = path.resolve(__dirname, '../src/components/r3f/daegu')
const { buildDuryuLandmark, buildMarketQuarter } = load(path.join(root, 'MarketQuarter.ts'))
fs.mkdirSync(path.join(root, 'baked'), { recursive: true })
for (const portrait of [false, true]) {
  for (const [name, build] of [['tower', buildDuryuLandmark], ['quarter', buildMarketQuarter]]) {
