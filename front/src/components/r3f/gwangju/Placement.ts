...
export function place(kit: CityBuilder, at: Point, yaw: number, scale: number, build: (local: CityBuilder) => void) {
  ...
  build({ add,
    box: (finish, size, position, rotation = [0, 0, 0], surface) => add(finish,
      new THREE.BoxGeometry(...size).applyMatrix4(new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(...rotation))).translate(...position), surface),
    beam: (finish, a, b, width, depth = width, surface) => {
      const start = new THREE.Vector3(...a), end = new THREE.Vector3(...b), direction = end.clone().sub(start)
      add(finish, new THREE.BoxGeometry(width, direction.length(), depth)
